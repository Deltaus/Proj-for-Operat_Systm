...
//		} while (c != 'q');
	...
	public static void increNumProcess() {
		numProcess.acquire();
		numRunProcess++;
		numProcess.release();
	}

	public static void decreNumProcess() {
		numProcess.acquire();
		numRunProcess--;
		numProcess.release();
	}
	...
	public static int[] allocatePages(int num) {
		...
		int[] allocatedPages = new int[num];
		for(int i = 0; i < num; i++) {
			allocatedPages[i] = freePages.removeFirst();
		}
		lock.release();
		return allocatedPages;
	}

	public static void recyclePages(int index) {
		lock.acquire();
		freePages.addLast(index);
		lock.release();
	}

	public static boolean isFreePagelistEmpty() {
	...
	public static int numRunProcess = 0;

	public static Lock PIDLock;

	public static Lock numProcess;
