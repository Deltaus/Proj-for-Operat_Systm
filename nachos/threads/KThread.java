...
public class KThread {
     ...
     *
     * @return the id given to this thread.
     */
    public int getId() {
	    return id;
    }
	...
	public static void finish() {
		...
		KThread caller = currentThread.joinCaller;
		if (caller != null) {
            caller.ready();
	...
	public void join() {
		...
		if (this.status != statusFinished) {
            KThread caller = this.joinCaller;
            Lib.assertTrue(caller == null, "Join can be called on a thread at most once!!!");
...
//		new KThread(new PingTest(2)).setName("forked thread 0").fork();
		//new PingTest(0).run();
        joinTest2();
	}
...
//	record the joinCaller call the join
